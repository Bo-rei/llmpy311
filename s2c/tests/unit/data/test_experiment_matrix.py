...
def test_smoke_and_core_matrices_have_declared_unit_counts() -> None:
    smoke = load_gate_matrix(PROJECT_ROOT / "configs/experiments/protocol_v2/smoke_gate.yaml")
    core = load_gate_matrix(PROJECT_ROOT / "configs/experiments/protocol_v2/gate_core_dense.yaml")
    assert len(smoke) == 36
    assert len(core) == 3300
...
def test_dry_run_uses_only_local_protocol_v2_export_paths(tmp_path: Path) -> None:
    ...
    spec = load_gate_matrix(PROJECT_ROOT / "configs/experiments/protocol_v2/smoke_gate.yaml")[0]
    payload = dry_run(paths, spec)
    assert payload["uses_textoir_data"] is False
    assert "textoir" not in payload["runtime_data_root"]
    assert all("textoir" not in item["path"] for item in payload["required_inputs"])
