...
真实 384 维判决计数为：Frozen `361 accepted / 2,445 rejected`；Trainable `27 accepted / 2,779 rejected`。其中 Frozen 的 `361` 个误接收有 `349` 个被 Trainable 修复为正确拒识。因而图中 Trainable 面板仍有大量红色 OOS 点是预期的正确拒识，不是错误划分；真正的误接收只用红色空心圆标出。

### 强解释

Frozen 面板中，边界附近的 OOS 误接收以空心圆集中出现；Trainable 面板中，空心圆明显减少，且大量样本被真实 384 维判定推到拒识侧。这个图支持“表示空间发生变化，并通过 Gate 边界影响决策”的解释；具体哪些样本跨界由 Fig. 3 和位移图负责。

### 解释边界

本图现在是保留径向距离的三维示意：方向来自白化表示的 PCA 投影，径向长度重设为该中心的真实 384 维归一化距离。因此三维球内外准确对应所选局部中心的接受边界，但角度和点间距离不能当作原始表示几何。图中的 Inside/Outside 只统计 OOS：Frozen 为 361/2445，Trainable 为 5/2801。Trainable 全 Gate 仍误接受 27 个，其中 22 个在当前球外、被其他中心接受，故红色空心圆可能出现在球外。该图不能用于直接比较左右面板的语义位移。二维 PCA 图仍保留为补充。
