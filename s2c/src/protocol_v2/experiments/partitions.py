...
def normalize_for_detector(embeddings: np.ndarray) -> np.ndarray:
    """Match the active detector's explicit L2 normalization step."""
...
class _StaticCenters:
    """Minimal sklearn-compatible center holder used by the active detector."""
...
def build_partition(
    ...
    ``kmeans`` deliberately matches the active detector: sorted intent order,
