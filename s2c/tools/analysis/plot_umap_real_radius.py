#!/usr/bin/env python3
"""Generate UMAP with calibrated real radius circles."""
...
def main():
    ...
    # Load detector centers and radii
    ...
    radii = np.asarray([s["radius"] for s in spheres], dtype=np.float64)
    ...
    # Select centers and radii
    selected_centers_mask = [c in selected_set for c in center_intents]
    selected_centers = centers[selected_centers_mask]
    selected_radii = radii[selected_centers_mask]
    print(f"Selected {len(selected_centers)} centers, radii range: {selected_radii.min():.2f} to {selected_radii.max():.2f}")
    ...
    # Calibrate radius
    known_mask = np.asarray([k == "known" for k in kinds])
    known_xy = sample_xy[known_mask]
    dists_to_centroids = np.linalg.norm(known_xy[:, None, :] - center_xy[None, :, :], axis=2)
    min_dists = dists_to_centroids.min(axis=1)
    avg_dist = np.mean(min_dists)
    visual_scale = 3.0
    avg_hd_radius = np.mean(selected_radii)
    umap_radii = (selected_radii / avg_hd_radius) * avg_dist * visual_scale
    print(f"Avg known-to-centroid UMAP dist: {avg_dist:.2f}")
    print(f"Calibrated UMAP radii range: {umap_radii.min():.2f} to {umap_radii.max():.2f}")
    ...
    # Centroids
    ax.scatter(center_xy[:, 0], center_xy[:, 1], c="#F1C40F", marker="*", s=180, linewidths=0.6, edgecolors="black", label="Centroids", zorder=5)

    # Draw calibrated radius circles
    for i, (cx, cy) in enumerate(center_xy):
        r = umap_radii[i]
        circle = plt.Circle((cx, cy), r, color=KIND_COLORS["known"], fill=False, linestyle="--", linewidth=0.8, alpha=0.4)
        ax.add_patch(circle)

    # Axis limits - tight to reduce whitespace
    all_xy = np.vstack([sample_xy, center_xy])
    x_min, y_min = all_xy.min(axis=0)
    x_max, y_max = all_xy.max(axis=0)
    x_range = x_max - x_min
    y_range = y_max - y_min
    ax.set_xlim(7.0, x_max + x_range * 0.05)
    ax.set_ylim(y_min - y_range * 0.07, y_max + y_range * 0.07)
    ...
    # Legend - 2x2 layout at lower left, transparent background, 9pt font, more vertical spacing
    handles = [
    ...
    ax.legend(handles=handles, loc="lower left", ncol=2, frameon=False, handletextpad=0.1, columnspacing=0.25, labelspacing=0.2, fontsize=9, bbox_to_anchor=(-0.02, 0.0), bbox_transform=ax.transAxes)

    fig.tight_layout()

    # Save PNG only
    png_path = OUTPUT_DIR / "gate_embedding_umap_real_radius.png"
