...
The proposed method uses a three-stage cascade: Gate $\rightarrow$ Router $\rightarrow$ Expert, as shown in Figure~\ref{fig:model_architecture}. Given an input utterance $x$, the Gate first determines whether $x$ belongs to the Known intent space or should be rejected as OOS. If $x$ is rejected, the Gate directly outputs an OOS label. Otherwise, $x$ is passed to the following modules. The Router predicts a coarse-grained domain, and the corresponding Expert assigns a Known intent label. Both downstream modules are closed-set classifiers trained on Known-intent observations, although their inference inputs may include OOS utterances misaccepted by the Gate. The Router and Experts use SmolLM-135M with low-rank adaptation (LoRA)~\citep{hu2022lora}.
...
\end{equation}
where $\mathcal{Y}_{\mathrm{K}}$ is the Known intent label set and $\mathcal{Y}_d$ is the intent subset for domain $d$. Gate-accepted inputs can include misaccepted OOS utterances. The closed-set Router and Experts assign these inputs a Known label without further rejection, so each remains a false negative for the OOS class.
...
For each centroid $c_{y,k}$, the Gate estimates a local radius $r_{y,k}$ from the training samples assigned to the corresponding region:
\begin{equation}
...
The distance function is a diagonal Mahalanobis distance. For an embedding $\bar{e}$ and a centroid $c_{y,k}$, the distance is computed as:
\begin{equation}
d(\bar{e},c_{y,k})=\sqrt{\sum_j\frac{(\bar{e}_j-c_{y,k,j})^2}{\sigma_{y,k,j}^{2}+\epsilon}},
\end{equation}
where $\bar{e}_j$ and $c_{y,k,j}$ denote the $j$-th components of the embedding and centroid, respectively. Moreover, $\sigma_{y,k,j}^{2}$ is the corresponding feature-wise variance estimated from the samples assigned to the local region, and $\epsilon$ is a small regularization constant. Compared with an unweighted distance, the diagonal Mahalanobis distance reduces the influence of dimensions with large within-region variation without estimating a full covariance matrix.
...
\begin{equation}
s(\bar{e})=\min_{y\in\mathcal{Y}_{\mathrm{K}},\,1\leq k\leq K_y}\frac{d(\bar{e},c_{y,k})}{r_{y,k}},
\label{eq:ryk}
\end{equation}
where $c_{y,k}$ denotes the $k$-th local centroid of Known intent $y$, and $r_{y,k}$ denotes the corresponding local boundary radius. The ratio in Eq.~(\ref{eq:ryk}) measures the distance from $\bar{e}$ to the nearest centroid relative to its learned boundary size.

The Gate decision is:
\begin{equation}
G(x)=
\begin{cases}
\mathrm{Known}, & s(\bar{e})\leq1,\\
\mathrm{OOS}, & s(\bar{e})>1.
\end{cases}
\end{equation}
The threshold is one because each distance is normalized by its corresponding radius. If $s(\bar{e})\leq1$, the input falls inside at least one learned Known-intent region and is accepted. Otherwise, the input lies outside all learned regions and is rejected as OOS. The Gate only determines whether an input belongs to the Known-intent space and does not assign a specific intent label.

For $M$ Known intents, $K$ centroids per intent and an embedding dimension of $h$, the boundary scoring requires $O(MKh)$ distance operations for each input, while storing the centroids and their diagonal variances requires $O(MKh)$ additional space. In the main configuration, $K=1$, so the additional boundary computation and storage grow linearly with the number of Known intents.
...
Moreover, for the proposed cascade workflow, the Gate uses the 22M-parameter \texttt{all-MiniLM-L6-v2}, while the Router and Expert modules use the 135M-parameter \texttt{SmolLM-135M}. For the Gate, only the last two Transformer blocks and the residual projection are trainable, resulting in about 3.75M trainable parameters. The Gate is optimized by AdamW with learning rates of $2\times10^{-4}$ and $2\times10^{-5}$ for the projection and MiniLM blocks, respectively, and the batch size is 64. In Eq.~(\ref{eq:rep_loss}), the intra-class and inter-class weights are both set as 0.1, the temperature is 0.07, and the margin is 0.20. For the Router and Expert, the LoRA ranks are set as 32 and 16, and the scaling factors are 64 and 32, respectively. Their batch size is 32 and the learning rate is $2\times10^{-4}$. The embedding dimension is set as $h=384$. The boundary configuration is selected for each dataset and KIR setting from the candidate centroid numbers, distance functions and boundary parameters using Known validation samples. The Gate is trained and selected without using real or pseudo OOS samples. The main results are repeated with three random seeds, i.e., 13, 42 and 87, and all experiments are conducted on an NVIDIA RTX 5070 GPU.
...
The performance on OOS intent detection is presented in Table~\ref{tab:main_results_all}. For different datasets and KIR settings, the proposed method selects the boundary configuration on the validation set while retaining the same Trainable MiniLM Gate and Gate--Router--Expert cascade workflow. The results of our method are reported over three random seeds.
...
& \textbf{Ours} & 78.41$\pm$1.63 & \textbf{95.18$\pm$0.48} & \textbf{91.18$\pm$0.65} & 80.27$\pm$0.37 & \textbf{95.50$\pm$0.96} & \textbf{91.76$\pm$0.87} & 53.64$\pm$3.79 & \textbf{89.07$\pm$2.16} & \textbf{81.66$\pm$3.17}\\
\midrule
\multirow{8}{*}{$0.50$}
...
& DA-ADB & \textbf{85.58} & 90.10 & 87.93 & \textbf{86.71} & 88.86 & \textbf{87.78} & \textbf{82.54} & 79.93 & \textbf{81.51}\\
& \textbf{Ours} & 82.98$\pm$0.79 & \textbf{92.13$\pm$0.81} & \textbf{88.23$\pm$0.87} & 83.13$\pm$1.50 & \textbf{89.23$\pm$2.04} & 86.33$\pm$1.59 & 61.86$\pm$4.26 & \textbf{83.55$\pm$3.85} & 75.69$\pm$4.68\\
\midrule
\multirow{8}{*}{$0.75$}
...
& DA-ADB & 88.43 & \textbf{86.00} & \textbf{87.39} & \textbf{87.66} & 74.55 & \textbf{83.56} & 85.93 & 69.37 & 81.12\\
& \textbf{Ours} & 84.71$\pm$0.64 & \textbf{86.80$\pm$1.55} & 85.58$\pm$1.07 & 85.04$\pm$1.06 & \textbf{75.94$\pm$1.93} & 82.28$\pm$0.96 & 66.34$\pm$0.80 & \textbf{79.91$\pm$1.67} & 73.28$\pm$1.51\\
\bottomrule
\end{tabular}
}
\caption{Performance comparison under different KIRs. For our method, the best validation-selected results within the proposed Trainable MiniLM cascade framework are reported over three random seeds.}
...
\subsection{System Ablation under Known-only Selection}

We first examine whether the explicit Gate and the Trainable MiniLM representation are necessary for the cascade workflow. Table~\ref{tab:ablation_all_kir} reports a fresh full-pipeline ablation under strict Known-only selection over seeds 13, 42 and 87. \textbf{Without Gate} removes the explicit geometric Gate and performs OOS rejection using downstream Expert confidence; its threshold is calibrated from Known validation samples only. \textbf{Frozen MiniLM} keeps the SmolLM Router/Expert components but replaces the Trainable Gate representation with the pretrained MiniLM. \textbf{Cascade-MiniLM} is the homogeneous MiniLM cascade: both its Gate and its closed-set Router/Expert heads use the Frozen MiniLM representation. \textbf{Cascade-SmolLM} uses a SmolLM prototype Gate and the fixed SmolLM downstream components. No real or pseudo OOS samples, and no test samples, are used for training or selection; auxiliary thresholds are selected by 90\% Known validation coverage.
...
\midrule
\multirow{5}{*}{$0.25$}
& Ours & \textbf{91.18$\pm$0.65} & \textbf{95.18$\pm$0.48} & \textbf{91.76$\pm$0.87} & \textbf{95.50$\pm$0.96} & \textbf{81.66$\pm$3.17} & \textbf{89.07$\pm$2.16}\\
& Frozen MiniLM & 84.39$\pm$1.69 & 90.52$\pm$0.97 & 86.08$\pm$3.05 & 91.60$\pm$2.21 & 70.74$\pm$6.36 & 80.98$\pm$5.00\\
& Without Gate & 54.15$\pm$5.00 & 61.96$\pm$5.78 & 51.41$\pm$6.52 & 56.22$\pm$7.94 & 62.04$\pm$3.30 & 73.64$\pm$3.03\\
& Cascade-MiniLM & 85.58$\pm$1.40 & 90.52$\pm$0.97 & 86.69$\pm$3.18 & 91.60$\pm$2.21 & 71.08$\pm$6.43 & 80.98$\pm$5.00\\
& Cascade-SmolLM & 68.34$\pm$2.80 & 77.25$\pm$2.36 & 29.38$\pm$1.63 & 21.41$\pm$2.03 & 32.39$\pm$1.28 & 40.74$\pm$1.91\\
\midrule
\multirow{5}{*}{$0.50$}
& Ours & \textbf{88.23$\pm$0.87} & \textbf{92.13$\pm$0.81} & \textbf{86.33$\pm$1.59} & \textbf{89.23$\pm$2.04} & \textbf{75.69$\pm$4.68} & \textbf{83.55$\pm$3.85}\\
& Frozen MiniLM & 80.18$\pm$0.36 & 84.40$\pm$0.50 & 75.51$\pm$3.52 & 76.83$\pm$5.51 & 61.80$\pm$2.22 & 70.48$\pm$2.21\\
& Without Gate & 66.13$\pm$1.65 & 66.33$\pm$2.38 & 64.50$\pm$0.26 & 58.10$\pm$0.58 & 66.05$\pm$4.37 & 74.34$\pm$4.21\\
& Cascade-MiniLM & 82.25$\pm$0.45 & 84.40$\pm$0.50 & 76.78$\pm$3.52 & 76.83$\pm$5.51 & 62.63$\pm$2.15 & 70.48$\pm$2.21\\
& Cascade-SmolLM & 64.08$\pm$1.50 & 64.36$\pm$2.20 & 45.44$\pm$1.52 & 18.14$\pm$2.32 & 40.43$\pm$0.81 & 43.52$\pm$0.79\\
\midrule
\multirow{5}{*}{$0.75$}
& Ours & \textbf{85.58$\pm$1.07} & \textbf{86.80$\pm$1.55} & \textbf{82.28$\pm$0.96} & \textbf{75.94$\pm$1.93} & \textbf{73.28$\pm$1.51} & \textbf{79.91$\pm$1.67}\\
& Frozen MiniLM & 76.08$\pm$0.49 & 72.72$\pm$0.80 & 76.63$\pm$0.92 & 63.51$\pm$3.78 & 61.13$\pm$1.70 & 65.72$\pm$2.51\\
& Without Gate & 71.52$\pm$1.64 & 60.93$\pm$3.50 & 75.00$\pm$0.86 & 54.01$\pm$4.06 & 67.58$\pm$1.24 & 73.03$\pm$1.44\\
& Cascade-MiniLM & 78.53$\pm$0.19 & 72.72$\pm$0.80 & 78.12$\pm$0.97 & 63.51$\pm$3.78 & 62.45$\pm$1.66 & 65.72$\pm$2.51\\
& Cascade-SmolLM & 70.42$\pm$1.52 & 60.85$\pm$3.40 & 62.80$\pm$1.18 & 14.93$\pm$2.86 & 48.99$\pm$0.52 & 48.60$\pm$1.03\\
\bottomrule
\end{tabular*}
\caption{Full-pipeline ablation under strict Known-only validation. Values are mean$\pm$std over seeds 13, 42 and 87. Bold numbers are column-wise maxima among the five variants.}
\label{tab:ablation_all_kir}
\end{table*}

As shown in Table~\ref{tab:ablation_all_kir}, removing the Gate causes a clear degradation in OOS detection, showing that downstream confidence alone is a weaker rejection mechanism. The homogeneous Cascade-MiniLM is also consistently below Ours because it uses the frozen MiniLM representation for both the Gate and the closed-set cascade. Cascade-SmolLM performs substantially worse in most settings, so a larger backbone does not guarantee a better boundary representation. These results isolate two effects: Trainable MiniLM improves the Gate geometry, while the downstream cascade determines the classification quality of samples accepted by that Gate. Gate-accepted OOS samples remain OOS false negatives even when a Router or Expert assigns them a Known label.

The Frozen MiniLM comparison further isolates the effect of representation adaptation. Its main difference appears in OOS false acceptance rather than a substantial increase in Known coverage. In particular, the improvement on StackOverflow becomes larger as KIR increases, where the frozen representation admits substantially more OOS samples into the Known region. Thus, fine-tuning the MiniLM representation mainly improves the geometry used by the boundary instead of merely shifting the rejection threshold.
